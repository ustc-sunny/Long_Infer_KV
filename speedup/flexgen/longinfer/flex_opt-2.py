...
from longinfer.adaptive_kv_manager import select_kv
...
class SelfAttention:
    def __init__(self, config, env, policy, layer_id, enable_prefetching, importance_ratio=0.2, chunk_size=32):
        self.config = config
        self.env = env
    ...
    def prefetch_cache(self, cache_home, cache_read_buf, i, prefetch_idx, prefetch_cache_stream):
        if i == 0:  # prefill, no cache
            return
        print("prefetch_cache_idx.shape[0]:", prefetch_idx.shape[0])
        k_home, v_home = cache_home.val
        # Pick code path
        if self.policy.compress_cache:
            path = 0
        ...
        if path == 0:  # Direct copy. #TODO:LongInfer 这里需要改一下prefetch_idx
            # shape: (s, b * n_head, head_dim)
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
                cache_write_buf, i, k, speculation_stream):
        ...
        if i == 0:  # prefill
            ...
            #(k_cache_next, _),(_,_) = next_cache_read_buf.pop()

            if self.enable_prefetching:
                #TODO:LongInfer这里需要根据下一层的K Cache和当前的Query来返回prefetch_idx
                h, new_k_cache, new_v_cache, self.prefetch_idx = self.compute.mha_gen(h, mask, w_q,
                    b_q, w_k, b_k, w_v, b_v, w_out, b_out, w_ln, b_ln, n_head,
                    k_cache, v_cache, donate, self.policy.attn_sparsity,
                    self.policy.compress_cache, self.policy.comp_cache_config, speculation_stream,importance_ratio=self.importance_ratio, chunk_size=self.chunk_size)
            else:
...
class OptLM:
    def __init__(self,
        ...
        layers = []
        self.attn_layer = []#存储attn层index
        layers.append(InputEmbed(self.config, self.env, self.policy))
        for i in range(self.config.num_hidden_layers):
            if policy.sep_layer:
                if (i == 0) or (i == (self.config.num_hidden_layers - 1)):
                    layers.append(SelfAttention(self.config, self.env, self.policy, i, False, importance_ratio=0.2, chunk_size=32))
                else:
                    layers.append(SelfAttention(self.config, self.env, self.policy, i, True, importance_ratio=0.2, chunk_size=32))
    ...
    def load_cache(self, i, j, k, overlap=True):
        ...
        if overlap:
            with torch.cuda.stream(self.load_cache_stream):
                if j not in self.attn_layer[2:]:#仅对非注意力层执行加载KV缓存
...
def get_test_inputs(prompt_len, num_prompts, tokenizer, path):
    prompts = []
...
def run_flexllmgen(args):
    ...
    # Task and policy
    warmup_inputs = get_test_inputs(32, num_prompts, tokenizer, args.warmup_input_path)
    inputs = get_test_inputs(prompt_len, num_prompts, tokenizer, args.test_input_path)
