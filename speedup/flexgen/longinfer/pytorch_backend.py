...
class TorchDevice:
    ...
    def mha(self, inputs, attention_mask, w_q, b_q, w_k, b_k, w_v, b_v,
        ...
        hidden = F.layer_norm(inputs.data, (h,), weight=w_ln.data, bias=b_ln.data)
        new_h = reform_hidden_states(hidden)#Ifinigen_modify
    ...
    def mha_gen(self, inputs, attention_mask, w_q, b_q, w_k, b_k, w_v, b_v,
                ...
                attn_sparsity, compress_cache, comp_config, partial_k_cache, speculation_stream, importance_ratio, max_num_kv, chunk_size,kv_abstract):
        ...
        if compress_cache:
            ...
            all_k = k_cache.device.decompress(k_cache)[:src_s-1]
        else:
            # shape: (s, b * n_head, head_dim)
            all_k = k_cache.data[:src_s-1]
        #print("ALL_K:", all_k.shape)
        current_q = q
        prefetch_idx = None
