...
from flexgen.pytorch_backend import (TorchTensor,TorchDevice, TorchDisk, TorchLink,
...
from longinfer.adaptive_kv_manager import (IAKM_attention,select_kv, set_partial_cache, weight_bias_concat, set_partial_weight)
...
class SelfAttention:
    def __init__(self, config, env, policy, layer_id, enable_prefetching, partial_weight_ratio=0.2, importance_ratio=0.2, max_num_kv = 400, chunk_size=32):
        self.config = config
        self.env = env
        ...
        self.prefetch_kv = None #要预取的KV数据
        self.kv_abstract = None #KV的抽象数据
        self.kv_abstract_idx = None #要预取的KV的抽象数据index
        self.max_num_kv = max_num_kv#self.policy.importance_ratio * prompt_len
        ...
        if self.layer_id > 1:
            ...
            self.partial_weight_ratio = 0.5

    def set_task(self, task):
        self.task = task
        #self.max_num_kv = int((self.task.gen_len+task.prompt_len)* self.importance_ratio)
    ...
    def init_cache_one_gpu_batch(self, cache_home):#TODO:LongInfer在kv cahce加载的初始化，比例的设置
    ...
    def load_cache(self, cache_home, cache_read_buf, i):#TODO:LongInfer加载KV Cache以进行计算
        ...
        if path == 0:  # Direct copy
            ...
            #TODO: LongInfer在拷贝的时候，可以在这里计算先从diskload到CPU数据，计算传输延迟，不同的baseline
    ...
    #LongInfer的prefetching
    #cache_read_buf：缓存读取缓冲区 i：当前解码步骤 
    def prefetch_cache(self, cache_home, cache_read_buf, i, prefetch_idx, prefetch_cache_stream):
        ...
        if path == 0:  # Direct copy
            ...
            if self.policy.attn_sparsity >= 1.0:
                #self.kv_abstract_idx
                #TODO:LongIner这里需要获取KV Abstract;baseline获取全部KV Data from disk

                self.prefetch_kv.data[0, :prefetch_idx.shape[0]], self.prefetch_kv.data[1, :prefetch_idx.shape[0]] = select_kv(prefetch_idx, k_home.data, v_home.data)#这里需要重新设计这个kv_select
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
                cache_write_buf, i, k, warmup, partial_weight_read_buf, partial_cache_read_buf, speculation_stream, prev_partial_cache_read_buf, prev_partial_weight_read_buf, weight_home, kv_abstract):
        n_head = self.config.n_head

        donate = [False] * 14
        h, donate[0] = hidden.val, True
        head_dim = h.shape[-1] // n_head#InfiniGen的设置
        ...
        if self.enable_prefetching and (i > 0):#Infinigen_modify 部分权重
        ...
        if i == 0:  # prefill
            ...
            #LongInfer: KV_abstract确定哪些KV cache被缓存了
        else:  # decoding
            mask, donate[1] = attention_mask.val.smart_copy(self.attention_compute)
            (k_cache, donate[12]), (v_cache, donate[13]) = cache_read_buf.pop()
            if self.enable_prefetching:
                partial_k_cache = partial_cache_read_buf.val
            if self.enable_prefetching:#修改mha_gen函数在pytorch_beckend.py中
                print("Attn:chunk_size:", self.chunk_size)
                h, new_k_cache, new_v_cache, self.prefetch_idx, self.kv_abstract_idx = self.compute.mha_gen(h, mask, w_q,
                    b_q, w_k, b_k, w_v, b_v, w_out, b_out, w_ln, b_ln, n_head,
                    k_cache, v_cache, donate, self.policy.attn_sparsity,
                    self.policy.compress_cache, self.policy.comp_cache_config, partial_k_cache, speculation_stream, self.importance_ratio, self.max_num_kv, self.chunk_size, kv_abstract)
            else:
                h, new_k_cache, new_v_cache, _, _ = self.compute.mha_gen(h, mask, w_q,
                    b_q, w_k, b_k, w_v, b_v, w_out, b_out, w_ln, b_ln, n_head,
                    k_cache, v_cache, donate, self.policy.attn_sparsity,
                    self.policy.compress_cache, self.policy.comp_cache_config, None, None, self.importance_ratio, None, self.chunk_size, None)
            cache_write_buf.store((new_k_cache, new_v_cache))
...
class OptLM:
    def __init__(self,
        ...
        layers = []
        self.attn_layer = []#LongInfer记录attention layer的层index
        layers.append(InputEmbed(self.config, self.env, self.policy))
        for i in range(self.config.num_hidden_layers):
            if policy.sep_layer:
                if (i == 0) or (i == (self.config.num_hidden_layers - 1)):
                    layers.append(SelfAttention(self.config, self.env, self.policy, i, False, partial_weight_ratio, importance_ratio, max_num_kv, chunk_size=32))
                else:
                    layers.append(SelfAttention(self.config, self.env, self.policy, i, True, partial_weight_ratio, importance_ratio, max_num_kv, chunk_size=32))
        ...
        self.speculation_stream = torch.cuda.Stream()#计算重要KV的cuda流
        self.prefetch_cache_stream = torch.cuda.Stream()#预取的cuda流
        ...
        if overlap:
            with torch.cuda.stream(self.load_cache_stream):
                if j not in self.attn_layer[2:]:
                    ...
                    #self.layers[j+1].load_cache(self.cache_home[j+1][k], self.cache_read_buf[j+1][k], i)
        else:
            if j not in self.attn_layer[2:]:
                self.layers[j].load_cache(self.cache_home[j][k], self.cache_read_buf[j][k], i)
                #self.layers[j+1].load_cache(self.cache_home[j+1][k], self.cache_read_buf[j+1][k], i)
    ...
    def compute_layer(self, i, j, k):
        ...
        # 这里不用skewing offline,然后要计算并获取KV abstract
        # 这里是去执行attention计算，然后
        #forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        #        cache_write_buf, i, k, warmup, partial_weight_read_buf, partial_cache_read_buf, speculation_stream, prev_partial_cache_read_buf, prev_partial_weight_read_buf, weight_home, cache_read_buf_next, kv_abstract):
         #LongIner：这里需要提取出KV abstract
            ...
            if k ==0:
                ...
                if j in self.attn_layer[2:]:
                    ...
                    self.layers[j].forward(self.hidden[i][j][k], self.cache_read_buf[j][k],
                        ...
                        self.cache_write_buf[j][k], i, k, warmup_state, self.partial_weight_read_buf[j],self.partial_cache_read_buf[j][k],self.speculation_stream, self.partial_cache_read_buf[prev_attn][k], self.partial_weight_read_buf[prev_attn],self.weight_home[j], kv_abstract = None)
                else:
                    self.layers[j].forward(self.hidden[i][j][k], self.cache_read_buf[j][k],
                        self.weight_read_buf[j], self.attention_mask[k],
                        self.cache_write_buf[j][k], i, k, warmup_state, self.partial_weight_read_buf[j],self.partial_cache_read_buf[j][k],self.speculation_stream, None, None, self.weight_home[j], kv_abstract = None)
        else:
            self.layers[j].forward(self.hidden[i][j][k], self.cache_read_buf[j][k],
                self.weight_read_buf[j], self.attention_mask[k],
...
def run_flexgen(args):
    ...
    model = OptLM(opt_config, env, args.path, policy, partial_weight_ratio=0.2, importance_ratio=0.2, max_num_kv=400)
...
def add_parser_arguments(parser):
    ...
    #longinfer parameters
    parser.add_argument("--importance_ratio", type=float, default=0.2)
