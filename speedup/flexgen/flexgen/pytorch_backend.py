...
from longinfer.adaptive_kv_manager import (IAKM_attention,reform_hidden_states)
...
class TorchDevice:
    ...
    def mha_gen(self, inputs, attention_mask, w_q, b_q, w_k, b_k, w_v, b_v,
                ...
                attn_sparsity, compress_cache, comp_config, p_w_q, partial_k_cache, speculation_stream, alpha, importance_ratio, max_num_kv, chunk_size,kv_abstract):
        ...
        hidden = F.layer_norm(inputs.data, (h,), weight=w_ln.data, bias=b_ln.data)
        new_h = reform_hidden_states(hidden)#Infinigen_modify
        ...
        # IAKM attention来确定prefetch_idx
        if compress_cache:
            # shape: (s, b * n_head, head_dim)
            all_k = k_cache.device.decompress(k_cache)[:src_s]
        else:
            # shape: (s, b * n_head, head_dim)
            all_k = k_cache.data[:src_s]
        prefetch_idx = None
        kv_abstract_idx = None
        if p_w_q is not None:
            current_q = q
            with torch.cuda.stream(speculation_stream):
                prefetch_idx, kv_abstract_idx = IAKM_attention(new_h, p_w_q, partial_k_cache, n_head, alpha, max_num_kv, current_q, all_k, importance_ratio, chunk_size, kv_abstract)
        
        # with torch.cuda.stream(speculation_stream):#在这里确定要选择的重要token id
        #     prefetch_idx, kv_abstract_idx = IAKM_attention(current_q, all_k, importance_ratio, chunk_size, kv_abstract)
        ...
        if isinstance(k_cache, TorchTensor):
            if attn_sparsity >= 1.0:  # Dense attention
                ...
                if k.is_cuda:
                    ...
                    q = q.float().cpu()
                    k, v = k.float(), v.float()
                    value = self._attention_value(q, k, v, None,#attention_mask.data,
        ...
        return TorchTensor.create_from_torch(value, self), k_new, v_new, prefetch_idx, kv_abstract_idx
    # def mha_gen(self, inputs, attention_mask, w_q, b_q, w_k, b_k, w_v, b_v,
    #             w_out, b_out, w_ln, b_ln, n_head, k_cache, v_cache, donate,
