...
from longinfer.adaptive_kv_manager import (IAKM_attention,_select_kv,select_kv,set_partial_cache, weight_bias_concat, set_partial_weight)
...
class SelfAttention:
    def __init__(self, config, env, policy, layer_id, enable_prefetching, partial_weight_ratio=0.2, importance_ratio=0.2, alpha=4, max_num_kv = 400, chunk_size=32):
        self.config = config
        self.env = env
    ...
    def prefetch_cache(self, cache_home, cache_read_buf, i, prefetch_idx, prefetch_cache_stream):
        ...
        if path == 0:  # Direct copy
            ...
            if self.policy.attn_sparsity >= 1.0:
                ...
                #TODO:LongIner这里需要模拟传输KV Abstract和KV数据;baseline获取全部KV Data from disk
                LKA = 0
                if LKA == 1:
                    kv_abstract_ratio = self.importance_ratio * (1/self.chunk_size)
                    kv_disk_ratio = self.importance_ratio * self.importance_ratio
                    kv_data_size = k_home.data.nbytes + v_home.data.nbytes
                    #print(f"kv_data_size: {kv_data_size} bytes")
                    transfer_data_size = kv_data_size * (kv_abstract_ratio + kv_disk_ratio)
                    #print(f"transfer_data_size: {transfer_data_size} bytes")
                    disk_bandwidth = 7 * 1e9 # 7 GB/s
                    transfer_time = transfer_data_size / disk_bandwidth
                ...
                self.prefetch_kv.data[0, :prefetch_idx.shape[0]], self.prefetch_kv.data[1, :prefetch_idx.shape[0]] = _select_kv(prefetch_idx, k_home.data, v_home.data)#这里需要重新设计这个kv_select
        ...
        #LongInfer: KV_abstract确定哪些KV cache被缓存了
            ...
            if self.enable_prefetching:#修改mha_gen函数在pytorch_beckend.py中
                ...
                h, new_k_cache, new_v_cache, self.prefetch_idx, self.kv_abstract_idx = self.compute.mha_gen(h, mask, w_q,
                    ...
                    self.policy.compress_cache, self.policy.comp_cache_config, p_w_q, partial_k_cache, speculation_stream,self.alpha, self.importance_ratio, self.max_num_kv, self.chunk_size, kv_abstract)
            else:
                h, new_k_cache, new_v_cache, _, _ = self.compute.mha_gen(h, mask, w_q,
                    b_q, w_k, b_k, w_v, b_v, w_out, b_out, w_ln, b_ln, n_head,
                    k_cache, v_cache, donate, self.policy.attn_sparsity,
                    self.policy.compress_cache, self.policy.comp_cache_config, None, None, None, None, self.importance_ratio, None, self.chunk_size, kv_abstract)
            cache_write_buf.store((new_k_cache, new_v_cache))
        ...
        for i in range(self.config.num_hidden_layers):
            if policy.sep_layer:
                if (i == 0) or (i == (self.config.num_hidden_layers - 1)):
                    #__init__(self, config, env, policy, layer_id, enable_prefetching, partial_weight_ratio=0.2, importance_ratio=0.2, alpha=4, max_num_kv = 400, chunk_size=32):
                    layers.append(SelfAttention(self.config, self.env, self.policy, i, False, partial_weight_ratio, importance_ratio, alpha, max_num_kv, chunk_size=32))
                else:
                    layers.append(SelfAttention(self.config, self.env, self.policy, i, True, partial_weight_ratio, importance_ratio, alpha, max_num_kv, chunk_size=32))
...
def run_flexgen(args):
    ...
    model = OptLM(opt_config, env, args.path, policy, args.partial_weight_ratio, args.importance_ratio, args.alpha, args.max_num_kv)
